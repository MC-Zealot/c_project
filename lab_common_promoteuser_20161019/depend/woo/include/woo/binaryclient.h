...
ssize_t 
binary_client_send(binary_client_t *client, const char *data, uint32_t data_len, uint64_t log_id, bool reset_conn = false, long hash = -1, bool auto_conn = true);

ssize_t 
binary_client_recv(binary_client_t *client, char *buf, uint32_t *data_len, uint32_t buf_size, uint64_t *log_id, bool long_conn = true);

ssize_t 
binary_client_talk(binary_client_t *client, const char *req, uint32_t req_len,
		char *buf, uint32_t *data_len, uint32_t buf_size, uint64_t log_id, bool long_conn = true);
...
binary_client_pool_talk(binary_client_pool_t *pool, const char *req, uint32_t req_len,
		char *buf, uint32_t *data_len, uint32_t buf_size, uint64_t log_id, uint32_t hash,
