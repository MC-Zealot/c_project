#ifndef _SPIO_DB_INTERFACE_HEADER_
#define _SPIO_DB_INTERFACE_HEADER_
#include <stdlib.h>
#include <stdio.h>
#include <vector>
#include <string>
...
// spio only support longconn
class SpioDbInterface : public DbInterface{
	public:
		SpioDbInterface(const Db_Info_Struct& db_info_struct):
			DbInterface(db_info_struct){
                // spio long conn vec
				LOG_ERROR("spio conf path is  %s\n", db_info_struct_.path_);
                p_client_  = new Proxy(SPIO);
                assert(p_client_->Init(db_info_struct_.path_, 500)==0);
			}
		virtual ~SpioDbInterface(){
			if(p_client_ != NULL){
				delete p_client_;
			}
		} 
	protected:
		...
		int get_spio_result(uint64_t n_key, const char* req, char* &resp_buf){
			SvrIpPort svr_ip_port;
			get_ip_port(n_key, svr_ip_port);
			return get_spio_result(svr_ip_port, req, resp_buf);
		}
		int get_spio_result_str(uint64_t n_key, string& req, string &resp_buf){
			SvrIpPort svr_ip_port;
			get_ip_port(n_key, svr_ip_port);
			return get_spio_result_str(svr_ip_port, req, resp_buf);
		}

		int get_spio_result(const SvrIpPort& svr_ip_port, const char* req,  char* &resp_buf){
		...
		int get_spio_result_str(const SvrIpPort& svr_ip_port, string& req,  string &resp_buf){
			//uint16_t total_index = svr_ip_port.ip_index_ + svr_ip_port.port_index_ * ip_num_;
			//uint32_t log_id = 1234567;
	...
	public :
		...
		int s_get_str(uint16_t type_id, string& p_str_key, string &p_result, 
				char& split_char, char& second_split_char, uint64_t n_key = 0){ 
		...
		int get_i(uint16_t type_id, const char* &p_str_key, uint32_t n_item_key, char* &p_result,
			...
			initialize();
			get_spio_result(0, p_str_key, p_result);
		...
		int get_i_str(uint16_t type_id, string &p_str_key, uint32_t n_item_key, string &p_result,
			...
			initialize();
			get_spio_result_str(0, p_str_key, p_result);
		...
		int get_i(uint16_t type_id, uint64_t n_key, uint32_t n_item_key, char* &p_result,
			...
			get_spio_result(n_key, req, p_result);
		...
		int mget_i(uint16_t type_id, uint64_t n_keys[], uint32_t key_num, 
			...
			for(MapSplitIpPort::iterator it = map_ip_port.begin(); it != map_ip_port.end(); it++){
				...
				for(uint32_t i = 0; i < req_struct.num_; i ++){
					...
					char* p_result = NULL;
					get_spio_result(svr_ip_port, req, p_result);
