...
typedef struct _Db_Info_Struct{
	...
	// for nspio & spio
    char group_[MAX_BUFFER];
    char path_[MAX_BUFFER];
	...
	public:
		...
		_Db_Info_Struct(const _Db_Info_Struct& db_info_struct){
			...
			strcpy(group_, db_info_struct.group_);
			strcpy(path_, db_info_struct.path_);
		...
		int load_config(const char* file_name, const char* db_name){
			...
			// nspio group
            /*if(db_type_ == NSPIO_DB_TYPE){
                read_profile_string(db_name, "GROUP",
                        group_, MAX_BUFFER, "\0", file_name);
            }   
            // spio conf path
            if(db_type_ == SPIO_DB_TYPE){
                read_profile_string(db_name, "PATH",
                        path_, MAX_BUFFER, "\0", file_name);
            }   */
...
class DbInterface{
	...
	protected:
		...
		int get_ip_port(uint64_t n_key,  SvrIpPort& svr_ip_port){
			...
			//LOG_ERROR("db_info_struct_:%d\n", db_info_struct_.select_type_);
