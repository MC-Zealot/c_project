#include "fanstarget_work_interface.h"
#include <openssl/aes.h>

DYN_WORK(FansTargetWorkInterface);

//json_object_get_int 64位
int FansTargetWorkInterface::get_ip_code(const string& ip)
{
...
int FansTargetWorkInterface::init_attri_by_req(json_object*& body_json,ACCESS_INFO*& access_info,VEC_CAND& fans_tar_ad_vec)
{
...
int FansTargetWorkInterface::mark_func(ACCESS_INFO*& access_info,FANS_TAR_AD*& fans_tar_ad)
{
	string mark;
	...
	snprintf(buff,MARK_BUFF_SIZE,"%s_%s_%d_%d_%lu_%lu_%d_%lu_%s_%d_%d",fans_tar_ad->adid.c_str(),fans_tar_ad->mid.c_str(),cost,
				...
				access_info->exposure_id.c_str(),access_info->appid,deliver_type);
	//LOG_ERROR("buf:%s\n",buff);
	mark = string(buff);
	...
	fans_tar_ad->mark = access_info->feed_position + "_" + string((char*)mark_enc);
	fans_tar_ad->mark_origin = access_info->feed_position + "_" + mark;
	return 0;				
}


int FansTargetWorkInterface::enc_ad_attri(ACCESS_INFO*& access_info,VEC_CAND& fans_tar_result_vec)
{
...
int FansTargetWorkInterface::json_dump(json_object*& resp_json, VEC_CAND& fans_tar_result_vec,
...
int FansTargetWorkInterface::return_process(ACCESS_INFO*& access_info,VEC_CAND& fans_tar_vec,
...
int FansTargetWorkInterface::release(VEC_CAND& fans_tar_vec)
{
...
int FansTargetWorkInterface::work_core(json_object *req_json, char* &p_out_string, int& n_out_len, uint64_t req_id)
{
