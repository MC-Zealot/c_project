#ifndef _WAX_WORK_INTERFACE_HEADER_
#define _WAX_WORK_INTERFACE_HEADER_
...
class WaxWorkInterface : public WorkInterface{
	...
	public:
		WaxWorkInterface(DbCompany*& p_db_company, int interface_id):
		...
		int init_attri_by_req(json_object* &body_json, ACCESS_INFO*& access_info, VEC_CAND& wax_ad_vec);
		//encrypt mark
		int mark_func(ACCESS_INFO*& access_info, WAX_AD*& wax_ad_ptr);
		//encrypt auction_price
		int aes_enc(WAX_AD*& wax_ad);
		//encrypt mark & auction_price
		int enc_ad_attri(ACCESS_INFO*& access_info, VEC_CAND& wax_result_vec);
		int json_dump(json_object* & resp_json, VEC_CAND& wax_ad_result_vec, 
			string key,  ACCESS_INFO*& access_info, int num_limit);
		int return_process(ACCESS_INFO*& access_info,
			VEC_CAND& wax_ad_vec,VEC_CAND& wax_ad_result_vec, 
				char* &p_out_temp, int& n_out_len);
		//release memory
		int release(VEC_CAND& wax_ad_vec);
